  ...
  let translate env fdecl =
    ...
    let env = { env with local_index = string_map_pairs
            ...
            let size_stmfd = 4 (* Total size pushed using stmfd -4 *)
                ...
                and var_size = 4 (* right now doing only for integers *) in
            let get_var var = 
                (try 
...
let rec expr = function
      ...
      | Call (fname, actuals) ->  (try (StringMap.find fname env.function_index)
...
let entry_function = try (StringMap.find "main" function_indexes); []
